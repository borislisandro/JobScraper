...
| Sources and resume/persona UI source | Manifest-driven structured source fields, explicit robots/local-network controls, probe/test status, local PDF/DOCX import, image-only manual paste, immutable corrected resume versions, resume selection, and expanded persona filters/threshold/unknown policy source. Frontend workflow tests pass; Rust document tests remain link-unexecuted. |
